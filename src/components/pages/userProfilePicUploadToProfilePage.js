...
CustomDialog = ({ open, onClose }) => {
  ...
  const handleCloseDialog = () => {
      onClose(); // Call the onCancel prop and get the return value
  }
  ...
  const handleSaveCrop = async () => {
    if (editor) {
      ...
      // Resize the image to 500px by 500px
      ...
      try {
        ...
        if (blob) {
          ...
          if (response.status === 200) {
            setTimeout(() => {
              setCroppedImage(resizedImageDataUrl); // Set the resized image
              setIsCropDialogOpen(false);
              setLoading(false);
