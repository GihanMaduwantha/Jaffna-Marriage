...
const Navbar = () => {
  ...
  useEffect(() => {
    // Calculate the actual height of the navbar (you may need to adjust this)
    const actualHeight = document.querySelector('.navbar').scrollHeight;
    setNavbarHeight(80);
  }, []);
  ...
  return (
    ...
    <nav className={isMobile && isOpen ? 'navbar gradiantLightGoldBg fullScreenNavForMobile ' : 'navbar gradiantLightGoldBg expanded navbaranimated' } style={!isMobile ? { height: `${navbarHeight}px` } : {}} >
      ...
      <div className="navbar-container">
        ...
        {brandlogo &&(
          ...
          <div className={`navLogo animatedLogo`}>
           <img src={brandlogo} alt="logo" />
           <p style={{ fontSize: '10px',width:'270px',fontStyle:'italic',color:'white',textAlign:'center' }}>உங்கள் நன்மையே எங்கள் நோக்கம்</p>
         </div>
          </Link>
        </div>
        )}
        ...
        <div className={`navLinks notMobileNavItem navLinksVisible animated`}>

          <Link
            to="/browse"
            className="ml-4 text-xl text-yellow-600 hover:text-yellow-500 relative group"
            style={{ fontFamily: 'serif', marginRight: '10px', fontSize:'24px', fontWeight:'bold'  }}
          >
        ...
        {login === true ? (
          ...
          <div className="navBtns notMobileNavItem navbarloginanimated expanded">
            <Link
              to="/login"
              className="navBtn navLinksVisible animated"
          ...
          {login === true ? (
            <div className="logoutAndUser" >
              ...
              <Menu className="menuForMobile"
                ...
                anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}
              >
                <Link to={"/userProfile/" + profileUserId}  onClick={closeMenu} ><MenuItem onClick={handleClose}>
                  <Avatar sx={{ width: 36, height: 36 }} className={!isProfileIconClicked ? 'clickedIconButton' : ''}>
                    <div className="userProfileImage " >
                      {imageLoading && (
                        <div className="imageLoadingPlaceholder">
